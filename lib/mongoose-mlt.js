...
module.exports = exports = function moreLikeThis(schema, options) {
  ...
  var mlt = function(seed, query, fields, opts, callback) {
    var Model = this;
    Model._mltTfIdf(seed, function(err, scores) {
      if (err) return callback(err);
      query.$text = { $search: Model._mltMakeQuery(scores) };
  ...
  schema.static('mlt', function(seed, query, fields, opts, callback) {
    var Model = this;
    ...
    if (seed instanceof mongoose.Types.ObjectId || ('string' === typeof seed && mongoose.Types.ObjectId.isValid(seed))) {
      // Get model when using an ObjectId as a seed document
      Model.findById(seed, null, {lean: true}, function(err, model) {
        if (err) return callback(err);
        if (!model) return callback(null, []);
        opts.seed = model._id;
        mlt.call(Model, model, query, fields, opts, callback);
      });
    } else if (_.isObject(seed)) {
      if (seed._id) opts.seed = seed._id;
      mlt.call(Model, seed instanceof mongoose.Document ? seed.toJSON() : seed, query, fields, opts, callback);
    } else {
  ...
  schema.static('_mltParseQuery', function(query) {
    ...
    return _.chain(fields).map(function(field) {
      var term = query;
      // Find contents of nested fields
      _.each(field.split('.'), function(chunk) {
        if (_.isArray(term)) {
          // Extract multiple values from array
          term = _.compact(_.pluck(term, chunk));
        } else if (term) {
          term = term[chunk];
        }
      });
      if (_.isNumber(term) || !_.isEmpty(term)) {
        // Tokenize string before returning
        return tokenizer.tokenize(term.toString());
      }
    }).flatten().compact().invoke('toLowerCase').difference(natural.stopwords).value();
  });
  ...
  var __countCached = function(Model, conditions, cb) {
    if (typeof(conditions) === 'function') {
      cb = conditions;
      conditions = {}
    }

    conditionsSerialized = JSON.stringify(conditions);
    if (Model.__countCache[conditionsSerialized] != null) {
      return cb(null, Model.__countCache[conditionsSerialized]);
    }
    Model.count(conditions, cb);
  };

  var __clearCache = function (Model) {
    times = Object.keys(Model.__countCacheCreatedIndex);
    now = new Date();
    times.forEach(function (time) {
      if (now - time > 3 * 60 * 1000) {
        delete Model.__countCache[Model.__countCacheCreatedIndex[time]];
        delete Model.__countCacheCreatedIndex[time];
      }
    });
  };
  ...
  schema.static('_mltIdf', function(terms, callback) {
    var Model = this;
    if (Model.__countCache == null) {
      Model.__countCache = {};
    }
    if (Model.__countCacheCreatedIndex == null) {
      Model.__countCacheCreatedIndex = {};
    }
    if (Model.__countCacheClearInterval == null) {
      Model.__countCacheClearInterval = setInterval(function () {
        __clearCache(Model)
      }, 10 * 1000)
    }
    ...
    __countCached(Model, function(err, count) {
      if (err) return callback(err);
      _corpus = count;
    ...
    function __idf(corpus, matching) {
      ...
      var idf = {};
      if (_.isEmpty(matching)) return callback(null, idf);
  ...
  schema.static('_mltTfIdf', function(terms, callback) {
    ...
    // If there are no terms, query for anything
    if (!terms.length) {
